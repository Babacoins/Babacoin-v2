...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"babaaddress\"\n"
            "\nReturn information about the given bitg address.\n"
            "\nArguments:\n"
            "1. \"babaaddress\"     (string, required) The bitg address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"babaaddress\", (string) The bitg address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: BABA address and we have full public key:
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"babaaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"babaaddress\"  (string, required) The bitg address to use for the signature.\n"
