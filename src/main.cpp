...
#if defined(NDEBUG)
#error "Babacoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ubaba) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    else if (nHeight <= 200 && nHeight > 100)
        nSubsidy = 320 * COIN;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // BABA: We use certain sporks during IBD, so check to see if they are
