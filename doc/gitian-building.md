...
*Setup instructions for a gitian build of Babacoin using a Debian VM or physical system.*
...
- [Building Babacoin](#building-babacoin)
...
Clone the git repositories for babacoin and gitian and then checkout the babacoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the babacoin repository
...
To build Babacoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the babacoin repository.
...
For example:
```bash
URL=https://github.com/konez2k/babacoin.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/babacoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/babacoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/babacoin-build.assert
```
